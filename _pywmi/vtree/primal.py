...
class PrimalGraph:
    ...
    def get_fill(self, node) -> List[Tuple[any, set]]:
        """ Get fill edges of node  (for each neighbor, the other neighbors that still require a connection)  """
        neighbors = self.connected_to.get(node)

        # connect neighbors
        if self._fills is not None:
            _, edges = self._fills.get(node)
            return edges
        else:
            fills = []
            for neighbor in neighbors:
                new_neighbors = neighbors - self.connected_to[neighbor]
                new_neighbors.discard(neighbor)
                fills.append((neighbor, new_neighbors))
            return fills
